...
export default function Header() {
  return (
    <div class="flex flex-col flex-wrap items-center px-8 py-6 mx-auto  md:flex-row tails-selected-element">
      <a href="/" class="flex items-center order-first mb-4 font-medium text-gray-900 lg:order-none lg:w-auto title-font lg:items-center lg:justify-center md:mb-0">
        <span class="text-xl font-black leading-none text-gray-900 select-none logo">
          <button
            class="inline-flex items-center justify-center h-10 px-6 font-light bg-gray-900 tracking-wide text-white transition duration-200  hover:bg-gray-900 focus:shadow-outline focus:outline-none"
            data-rounded="rounded-lg"
            data-primary="gray-900">
            <Link to="/">COACH DIPAN</Link>
          </button>
        </span>
      </a>
      <nav class="flex flex-wrap items-center justify-center text-base font-light tracking-tight md:ml-auto">
        {menu.map((item, index) => (
          <Link to={item.href} className="mx-5 hover:text-gray-900">
            {item.name}
          </Link>
        ))}
      </nav>
